...
def train_optimization_models():
    ...
    Note on data: this uses a synthetic proxy dataset (see generate_dataset.py
    for the full methodology note), since real proprietary plant telemetry is
    not publicly available. Models are validated on a held-out 20% test split
    so the reported accuracy reflects genuine generalization, not training fit.
    """
    np.random.seed(42)
    n_samples = 1200
    ...
    # Held-out validation split (models are trained on X_train only)
    X_train, X_test, moisture_train, moisture_test = train_test_split(
        X, moisture, test_size=0.2, random_state=42
    )
    _, _, sec_train, sec_test = train_test_split(
        X, sec, test_size=0.2, random_state=42
    )

    model_moisture = RandomForestRegressor(n_estimators=60, random_state=42).fit(X_train, moisture_train)
    model_sec = RandomForestRegressor(n_estimators=60, random_state=42).fit(X_train, sec_train)

    # Held-out test metrics (genuine generalization performance)
    moisture_pred_test = model_moisture.predict(X_test)
    sec_pred_test = model_sec.predict(X_test)

    metrics = {
        "moisture_r2": r2_score(moisture_test, moisture_pred_test),
        "moisture_rmse": mean_squared_error(moisture_test, moisture_pred_test) ** 0.5,
        "sec_r2": r2_score(sec_test, sec_pred_test),
        "sec_rmse": mean_squared_error(sec_test, sec_pred_test) ** 0.5,
        "n_train": len(X_train),
        "n_test": len(X_test),
    }

    # Refit on the FULL dataset for production-quality live predictions,
    # now that generalization has already been honestly measured above.
    model_moisture_full = RandomForestRegressor(n_estimators=60, random_state=42).fit(X, moisture)
    model_sec_full = RandomForestRegressor(n_estimators=60, random_state=42).fit(X, sec)

    return model_moisture_full, model_sec_full, metrics

model_moisture, model_sec, model_metrics = train_optimization_models()

# --- SIDEBAR: MODEL PERFORMANCE ---
with st.sidebar.expander("📈 Model Performance (held-out test set)", expanded=False):
    st.caption(f"Trained on {model_metrics['n_train']} samples · validated on {model_metrics['n_test']} unseen samples")
    st.markdown(
        f"**Moisture Model**  \n"
        f"R² = `{model_metrics['moisture_r2']:.3f}`  ·  RMSE = `{model_metrics['moisture_rmse']:.2f} pp`"
    )
    st.markdown(
        f"**SEC Model**  \n"
        f"R² = `{model_metrics['sec_r2']:.3f}`  ·  RMSE = `{model_metrics['sec_rmse']:.2f} kWh/t`"
    )
    st.caption("Trained on a physics-informed synthetic proxy dataset — see generate_dataset.py for methodology.")
